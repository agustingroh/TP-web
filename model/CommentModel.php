...
class CommentModel
{
    ...
    {
        $stmt = $this->db->prepare("SELECT c.id_comment, c.comment, c.punctuation, u.email, p.component, c.date FROM Comment c INNER JOIN Product p ON c.id_product=p.id_product INNER JOIN User u ON u.id_user=c.id_user WHERE p.id_product=? ORDER BY c.date desc");
