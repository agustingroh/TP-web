...
class ProductController
{
            ...
            if (isset($_SESSION['email']) && $_SESSION['role'] == UserRole::ADMIN)  {
                if (isset($_POST['product']) && (isset($_POST['price'])) && (isset($_POST['description'])) && (isset($_POST['brand'])) && !empty($_POST['description']) && !empty($_POST['price']) && !empty($_POST['product'])) {
                    ...
                    if ($_FILES['image']['name'] != ""){
                        ...
                        //no cambio imagen
                        $this->productModel->editWithOutFilepath($_POST['id'], $_POST['product'], $_POST['description'], $_POST['brand'], $_POST['price']);
                        header("Location: ".BASE_URL  . "admins");
                    }
    ...
    public function getProduct($id)
    {
        try {  
            ...
            else  $this->adminView->showMessage("Ese producto no existe", 400);       
        } catch (Exception $e) {
            $this->adminView->showMessage("internal server error", 500);
        }
    }
